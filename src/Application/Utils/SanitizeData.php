...
class SanitizeData
{
    ...
    {
        $string = self::indexOf(self::$IGNORE_FILTER, 'strip_tags') === -1 ? strip_tags($string) : $string;
        $string = self::indexOf(self::$IGNORE_FILTER, 'htmlspecialchars') === -1 ? htmlspecialchars($string) : $string;
        $string = self::indexOf(self::$IGNORE_FILTER, 'trim') === -1 ? trim($string) : $string;
        $string = self::indexOf(self::$IGNORE_FILTER, 'stripslashes') === -1 ? stripslashes($string) : $string;
        return $string;
    }
    ...
    protected static function cleanAllData()
    {
        if (!isset(self::$DATA)) {
            ...
            foreach ($requestMethod as $key => $value) {
                self::$DATA[$key] = self::indexOf(self::$IGNORE_KEY, $key) === -1 ? self::sanitize($value) : $value;
            }
     ...
     */
    protected static function indexOf(array $array, string $word)
    {
